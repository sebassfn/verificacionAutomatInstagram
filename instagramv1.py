# -*- coding: utf-8 -*-
""" Created on Sat Dec 28 22:04:43 2019 @author: sebas """
...
def obtenerLinksPublicacionesInstagram(username):
    ...
    while cnt < max_cnt:
        if cnt == 0:
            texto_variable="window.scrollTo(0, document.body.scrollHeight/1.5);"
        else:
            texto_variable ="window.scrollTo(document.body.scrollHeight/" + str(1.5*cnt) + ", document.body.scrollHeight/" + str(1.5*(cnt+1)) + ");"

#        print(texto_variable)
        Pagelength = browser.execute_script(texto_variable)
        cnt += 1
...
def enviar_Notif_si_ecuentra_info(url, mensaje_a_buscar):
...
#        print(datos)
    ...
    for palabra in datos.split():
        if palabra == mensaje_a_buscar:
            print("ECONTRADO: --- ", mensaje_a_buscar, " ---")
            print(datos)

def busqueda_info_Publicaciones_Inst(username,mensaje_a_buscar):
    urls = obtenerLinksPublicacionesInstagram(username)
    for url in urls:
        enviar_Notif_si_ecuentra_info(url, mensaje_a_buscar)
